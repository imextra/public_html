...
abstract class CBaseEntity
{
	...
	public function GetList($select, $arFilter = array(), $group = array(), $order = array(), $limit = array(), $options = array())
	{
		global $DB;
		...
		list($bsqlWhere, $bsqlHaving, $bsqlHavingGroup) = $this->BuildArWhere($arFilterChains, $arFilter, $aliases);
		...
		if (!empty($groupChains) || $need_group_by || !empty($bsqlHavingGroup))
		{
			$bsqlGroup = $this->BuildGroup($groupChains, $aliases, $selChains, $ordChains);

			if (!empty($bsqlHavingGroup))
			{
				$bsqlGroup = empty($bsqlGroup) ? $bsqlHavingGroup : $bsqlGroup . ', '.$bsqlHavingGroup;
			}
...
//			}
	...
	protected function preBuildArWhere(&$arFilter, &$arFilterChains, &$aliases, &$cswFields, $level = 0, $useHaving = false)
	{
		...
		if ($level == 0)
		{
			// put all filters in having if having already exists and root logic is OR
			if (!empty($havingFilter) && $logic == 'OR')
			{
				$havingFilter = array_merge($havingFilter, $whereFilter);
				$whereFilter = array();
			}
	...
	protected function BuildArWhere($arFilterChains, $arFilter, $aliases)
	{
		...
		// if some fields moved to having, we need to group by them
		$strGroup = join(', ', $this->AddGroupFromHaving($havingFilter, $cswFields));

		return array($strWhere, $strHaving, $strGroup);
	}


	protected function AddGroupFromHaving($havingFilter, $cswFields)
	{
		$group = array();

		foreach ($havingFilter as $strFilter => $filterMatch)
		{
			if ($strFilter === 'LOGIC')
			{
				continue;
			...
			if (!is_numeric($strFilter))
			{
				...
				if (!CExprEntityField::checkAggregation($strChain))
				{
					// !! bug: here also can be expr without aggr or expr with aggr- and noaggr fields
					$group[] = $cswFields[$strChain]['FIELD_NAME'];
				}
			}
			elseif (is_array($filterMatch))
			{
				$group = array_merge($group, $this->AddGroupFromHaving($filterMatch, $cswFields));
			}
		}
		...
		foreach ($addChains as $strChain => $chainInfo)
		{
			...
			if ($lastElem['value'] instanceof CExprEntityField)
			{
				...
				foreach ($bfChains as $bfChainInfo)
				{
					...
					if ($bfLastElem['value'] instanceof CExprEntityField)
					{
						$sql[] = $bfLastElem['value']->GetSQLDefinition(false);
					}
					else
					{
