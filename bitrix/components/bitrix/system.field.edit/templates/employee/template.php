			...
			if (document.getElementById('<?echo $name_x?>'))
			{
				...
				if (value_<?=$name_x?> > 0)
				{
					if (old_value != value_<?=$name_x?>)
					{
						...
						{
							DV_<?=$name_x?>.innerHTML = '[<a title="<?echo CUtil::JSEscape(GetMessage("MAIN_EDIT_USER_PROFILE"))?>" class="tablebodylink" href="/bitrix/admin/user_edit.php?ID=<?echo $USER->GetID()?>&lang=<?echo LANG?>"><?echo $USER->GetID()?></a>] (<?echo CUtil::JSEscape(htmlspecialchars($USER->GetLogin()))?>) <? echo CUtil::JSEscape(htmlspecialchars($USER->GetFirstName().' '.$USER->GetLastName()))?>';
						}
					}
