...
if(!$arParams['ACCESS']['VIEW_GROUPS']){
	...
	$arResult['TF']['FIRST'] = array('ID','NAME','PREVIEW_PICTURE','DETAIL_TEXT','PREVIEW_TEXT','ACTIVE');
	$arResult['TF']['SECOND'] = array('ADDRESS','KINOAFISHA_ID','KINOAFISHA_NAME','PHONE','SITE','BOROUGH_ID','METRO_ID','NAME_ENG','CINEMA_NETWORK_ID');
	...
	if(!empty($arParams["IBLOCK_ID_METRO"])){
		...
		$arFilter = array(
			"IBLOCK_ID" => $arParams["IBLOCK_ID_METRO"],
			"CHECK_PERMISSIONS" => "Y",
			"ACTIVE" =>'Y',
		...
		while($obElement = $rsElement->GetNextElement())
		{
			...
			$arResult['METRO'][$arTempTemp['ID']] = $arTempTemp;
	...
	$arResult['BOROUGH'] = array();
	if(!empty($arParams["IBLOCK_ID_BOROUGH"])){
		...
		$arSelect = array_merge($arSelect,$arResult['TF2']['FIRST']);
		$arFilter = array(
			"IBLOCK_ID" => $arParams["IBLOCK_ID_BOROUGH"],
			"CHECK_PERMISSIONS" => "Y",
			"ACTIVE" =>'Y',
			"PROPERTY_CITY" => $arParams['CITY']['ID'],
		);
		...
		while($obElement = $rsElement->GetNextElement())
		{
			...
			$arResult['BOROUGH'][$arTempTemp['ID']] = $arTempTemp;
	...
	$arResult['CINEMA_NETWORK'] = array();
	if(!empty($arParams["IBLOCK_ID_CINEMA_NETWORK"])){
		$arResult['LOG'][] = 'Формируем SELECT запрос...';
		$arSelect = array(
			'IBLOCK_ID',
			// 'PROPERTY_*',
		...
		$arSelect = array_merge($arSelect,$arResult['TF2']['FIRST']);
		$arFilter = array(
			"IBLOCK_ID" => $arParams["IBLOCK_ID_CINEMA_NETWORK"],
			"CHECK_PERMISSIONS" => "Y",
			"ACTIVE" =>'Y',
		);
		$arNav = false;
		...
		while($obElement = $rsElement->GetNextElement())
		{
			...
			$arResult['CINEMA_NETWORK'][$arTempTemp['ID']] = $arTempTemp;
	...
	foreach($arResult['TF']['ALL'] as $tempTitle){
		...
		if($tempTitle == 'METRO_ID' && !empty($_POST["submitUpdateData"])){
			...
			if(emptyArray($_POST[$tempTitle])){
				...
				foreach($_POST[$tempTitle] as $tt){
					$tt = intval($tt);
					if(!emptyArray($arResult['METRO']) && !empty($tt) && isset($arResult['METRO'][$tt])){
						$arTempRet[] = $tt;
					}
				}
		...
		// Если никакой пункт не выбрать то его не будет в _POST, поэтому делаем проверку
		if($tempTitle == 'BOROUGH_ID' && !empty($_POST["submitUpdateData"])){
			...
			if(emptyArray($_POST[$tempTitle])){
				...
				foreach($_POST[$tempTitle] as $tt){
					$tt = intval($tt);
					if(!emptyArray($arResult['BOROUGH']) && !empty($tt) && isset($arResult['BOROUGH'][$tt])){
						$arTempRet[] = $tt;
					}
				}
				!emptyArray($arTempRet) ? $_POST[$tempTitle] = $arTempRet : $_POST[$tempTitle] = array();
	...
	if(!empty($_POST['submitUpdateData']) && $arParams['ACCESS']['UPDATE_DATA']){
		...
		if(empty($arResult['ITEM']['~PROPERTIES']['CITY'])){
			if(!empty($arParams['CITY']['ID'])){
				$arResult['ITEM']['~PROPERTIES']['CITY'] = $arParams['CITY']['ID'];
				$arResult['LOG'][] = 'Указываем город по умолчанию';
			}else{
				$arResult['ERRORS'][] = 'Не смогли найти город.';
				$arResult['LOG'][] = 'Ошибка: Не смогли найти город.';
			}
		}
